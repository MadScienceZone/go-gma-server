...
func TestDiceProbabilities(t *testing.T) {
	d4, err1 := NewDiceBasic(1, 4, 0)
	d6, err2 := NewDiceBasic(1, 6, 0)
	d8, err3 := NewDiceBasic(1, 8, 0)
	d10, err4 := NewDiceBasic(1, 10, 0)
	d12, err5 := NewDiceBasic(1, 12, 0)
	d20, err6 := NewDiceBasic(1, 20, 0)
	dpct, err7 := NewDiceBasic(1, 100, 0)
...
func TestDiceStrProbabilities(t *testing.T) {
	...
	d6, err2 := NewDice("d6")
	d8, err3 := NewDice("d4")
	d10, err4 := NewDice("d10")
	d12, err5 := NewDice("d12")
	d20, err6 := NewDice("d20")
	dpct, err7 := NewDice("d%")
...
func TestDiceCons(t *testing.T) {
	d, err := NewDiceByParameters(1, 2, 3, 4, 5, 0)
...
func TestDiceBellcurves(t *testing.T) {
	_3d6, err := NewDiceBasic(3, 6, 0)
	if err != nil { t.Fatalf("Construct 3d6: %v", err) }

	s3d6, err := NewDice("3d6")
	if err != nil { t.Fatalf("Construct s3d6: %v", err) }

	_5d10plus3, err := NewDiceBasic(5, 10, 3)
	if err != nil { t.Fatalf("Construct 5d10+3: %v", err) }

	s5d10plus3, err := NewDice("5d10+3")
	if err != nil { t.Fatalf("Construct s5d10+3: %v", err) }
...
func compareResults (a, b []StructuredResult) bool {
	...
	for i := range a {
		...
		if len(a[i].Details) != len(b[i].Details) { return false }
		for j := range a[i].Details {
			if a[i].Details[j] != b[i].Details[j] { return false }
		}
	}
	return true
}
